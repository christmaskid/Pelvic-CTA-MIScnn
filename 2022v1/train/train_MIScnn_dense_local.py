...
data_aug_local = Data_Augmentation(cycles=2, scaling=True, rotations=True, elastic_deform=True, mirror=True,
                             brightness=True, contrast=True, gamma=True, gaussian_noise=True)
...
train_list_local = sample_list_local_1[0::5]+sample_list_local_1[1::5]+sample_list_local_1[2::5]
val_list_local = sample_list_local_1[3::5]
test_list_local = sample_list_local_1[4::5]
...
pp_local = Preprocessor(data_io_local_1, data_aug=data_aug_local, batch_size=1, subfunctions=subfunctions, prepare_subfunctions=True, 
                  prepare_batches=False, analysis="patchwise-crop", patch_shape=(128, 256, 256))
...
model = Neural_Network(preprocessor=pp, architecture=unet_dense, loss=new_loss_3, metrics=metric_list, batch_queue_size=2, workers=2, learninig_rate=0.00008)
...
model.dump("unet_dense_30_v3_uf.hdf5")
...
model_local = Neural_Network(preprocessor=pp_local, architecture=unet_dense, loss=new_loss_3, metrics=metric_list, batch_queue_size=2, workers=2, learninig_rate=0.00008)
model_local.load("unet_dense_30_v3_uf_best.hdf5")
history = model_loca1.evaluate(training_samples=train_list_local,
                         validation_samples=val_list_local,
                         epochs=30,
                         iterations=None,
                         callbacks=[cb_lr, cb_cp_local]
                         )
