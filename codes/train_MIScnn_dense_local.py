...
interface_local = My_DICOM_interface(structure_dict = structure_dict, classes=2, annotation_tag="vessels", local=True, local_range=[0.2, 0.7])
...
train_list_local = sample_list_local[0::5]+sample_list_local[1::5]+sample_list_local[2::5]
...
pp_local = Preprocessor(data_io_local, data_aug=data_aug, batch_size=1, subfunctions=subfunctions, prepare_subfunctions=True, 
                  prepare_batches=False, analysis="patchwise-crop", patch_shape=(128, 256, 256))
...
# CALLBACK
# Define Callbacks
#cb_lr = ReduceLROnPlateau(monitor='loss', factor=0.1, patience=2, verbose=1, mode='min', min_delta=0.001, cooldown=1, min_lr=1e-11)
cb_lr_local = ReduceLROnPlateau(monitor='loss', factor=0.1, patience=2, verbose=1, mode='min', min_delta=0.001, cooldown=1, min_lr=1e-15)
...
model_local = Neural_Network(preprocessor=pp_local, architecture=unet_dense, loss=new_loss_3, metrics=metric_list, batch_queue_size=2, workers=2, learninig_rate=0.00005)
model_local.load("unet_dense_30_v3_suf_best.hdf5")
