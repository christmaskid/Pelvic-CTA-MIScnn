# Modified by Alexander Shieh 2021/11/16, Yu-Tong Cheng 2022/05/24
...
class My_DICOM_interface(Abstract_IO):
    # Class variable initialization
    def __init__(self, channels=1, classes=2, three_dim=True, mask_background=0,
                 structure_dict={}, annotation_tag=None, local=False, local_range=[0,1]):
        ...
        Args:
            ...
            local (bool): Get the middle 256 * 256 voxels along x- and y- axes.
            local_range ([float, float]): Range of proportion along z-axis.
        Returns:
            None
        """
        ...
        self.local = local
        self.lower, self.upper = local_range[0], local_range[1]
    ...
    def load_image(self, index):
        ...
        img_array = img_array[int(img_array.shape[0]*self.lower):int(img_array.shape[0]*self.upper), :, :]
        if self.local == True:
            img_array = img_array[:, 128:384, 128:384]
        print(img_array.shape)
        return img_array, {"type":"dicom", "spacing":np.absolute(spacing)}
...
#         contours = self.get_ROI_data(rtStruct_path)
        ...
        dicom_image, image_data = self.load_image_sitk(index)
...
#         segmentations = self.convert(contours, images)
        ...
        coords_z, coords_y, coords_x = self.get_ROI_from_csv(rtstruct_file=rtStruct_path)
        
        for idx, z in enumerate(coords_z):
            try:
                np_mask[z, coords_y[idx], coords_x[idx]] = 1
            except:
                continue

        # Return segmentations
        np_mask = np_mask[int(np_mask.shape[0]*self.lower):int(np_mask.shape[0]*self.upper), :, :]
        if self.local == True:
            np_mask = np_mask[:, 128:384, 128:384]
        print(np_mask.shape)
        return np_mask
    
    def get_ROI_from_csv(self, rtstruct_file = None):
        coords_z =[]
        coords_y = []
        coords_x = []
        with open(rtstruct_file, 'r') as csvfile:
            reader = csv.reader(csvfile)
            for row in reader:
                coords_z.append(int(row[0]))
                coords_y.append(int(row[1]))
                coords_x.append(int(row[2]))
